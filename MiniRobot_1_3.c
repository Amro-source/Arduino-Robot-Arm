...
void loop()
{
  ...
  else if(playmode) // abspielen
  {
    if (Step) // next step aus array lesen
    {
      ...
      {
        arrayStep = 0; // 
        calc_pause(); // pausenlänge aus Clawnpoti lesen     
        countverz = 0;
...
void readPot()
{
   SensVal[0] = analogRead(sensorPin0); SensVal[0] += -10; // Shoulder drehen
...
void mapping()
{
  ist[0] = map(SensVal[0], 150, 900, 600, 2400);// Shoulder drehen
...
void move_servo()
{                         // Connections Mapped
  servo_0.writeMicroseconds(ist[3]); // Claw
...
void Button()
{
  ...
  {
    delay(20);
    if (digitalRead(6) == true) // tester released
    {
      if (Taster == 0)
      {
        Taster = 1;
